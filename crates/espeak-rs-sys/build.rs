...
const ESPEAK_NG_REV: &str = "d7b26b545cef4b1e7cac8923af2e560cbd7c93e2";
...
fn main() {
    println!("cargo:rerun-if-env-changed=ESPEAK_NG_SOURCE_DIR");
    ...
    let espeak_src = espeak_source_dir(&out_dir);
    ...
    let mut config = Config::new(&espeak_src);
...
fn espeak_source_dir(out_dir: &Path) -> PathBuf {
    if let Some(path) = env::var_os("ESPEAK_NG_SOURCE_DIR").map(PathBuf::from) {
        return path;
    }

    let espeak_dir = out_dir.join("espeak-ng");
    if !espeak_dir.exists() {
        let status = Command::new("git")
            ...
            .arg(&espeak_dir)
            .status()
            .expect("failed to spawn git clone for espeak-ng");
        assert!(status.success(), "failed to clone espeak-ng");
        let status = Command::new("git")
            .arg("-C")
            .arg(&espeak_dir)
            .args(["fetch", "--depth", "1", "origin", ESPEAK_NG_REV])
            .status()
            .expect("failed to fetch pinned espeak-ng revision");
        assert!(
            status.success(),
            "failed to fetch espeak-ng {ESPEAK_NG_REV}"
        );
        let status = Command::new("git")
            .arg("-C")
            .arg(&espeak_dir)
            .args(["checkout", "--detach", ESPEAK_NG_REV])
            .status()
            .expect("failed to checkout pinned espeak-ng revision");
        assert!(
            status.success(),
            "failed to checkout espeak-ng {ESPEAK_NG_REV}"
        );
    }
    espeak_dir
}
